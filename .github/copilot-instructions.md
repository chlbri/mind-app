...
**TOUJOURS** utiliser des exports nommés pour les composants et modules :
...
// ❌ MAUVAIS - Export nommé incorrect
export { MyComponent };
```
...
- Refactoring plus sûr (renommage de symboles)
- Erreurs de typage plus claires
- Cohérence dans tout le codebase

### Linting

**TOUJOURS** utiliser la commande suivante pour linter les fichiers :

```bash
pnpm run lint
```

**Ne JAMAIS** utiliser directement `npx eslint`, `eslint` ou toute autre
variante. La commande `pnpm run lint` est configurée spécifiquement pour ce
projet et inclut les bonnes options, patterns de fichiers et exclusions.
