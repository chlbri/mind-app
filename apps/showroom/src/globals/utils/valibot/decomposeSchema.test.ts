...
describe('#01 => decomposeSchema', () => {
    ...
    it('#03 => should return all fields at each level of nesting', () => {
      const schema = v.object({
        ...
        nested: v.object({
          ...
          deepNested: v.object({ deepLevel: v.boolean() }),
        }),
      });
  ...
  describe('#06 => Edge cases', () => {
    ...
    it('#02 => should handle object with only nested empty object', () => {
      const schema = v.object({ nested: v.object({}) });
  ...
  describe('#07 => Real world examples', () => {
    ...
    it('#02 => should decompose a configuration schema', () => {
      const schema = v.object({
        ...
        database: v.object({
          url: v.string(),
          pool: v.object({ min: v.number(), max: v.number() }),
        }),
      });
  ...
  describe('#08 => Deep nested schema with validation', () => {
      ...
      const schema = v.object({
        level1: v.object({
          level2: v.object({
            level3: v.object({
              level4: v.object({
                level5: v.object({ value: v.string(), count: v.number() }),
              }),
            }),
          }),
        }),
    ...
    it('#02 => should validate with empty object on deeply nested schema', () => {
      const schema = v.object({
        company: v.object({
          department: v.object({
            team: v.object({
              members: v.object({ name: v.string(), email: v.string() }),
            }),
          }),
        }),
      });
    ...
    it('#03 => should validate with partial data on deeply nested schema', () => {
      const schema = v.object({
        app: v.object({
          config: v.object({
            database: v.object({
              connection: v.object({ host: v.string(), port: v.number() }),
            }),
          }),
        }),
      });
    ...
    it('#04 => should validate with complete nested data', () => {
      const schema = v.object({
        root: v.object({
          branch: v.object({ leaf: v.object({ data: v.string() }) }),
        }),
      });

      const decomposed = decomposeSchema(schema);
      const completeData = { 'root.branch.leaf.data': 'test-value' };
  ...
  describe('#09 => Deep nested schema with non-empty object validation and failures', () => {
    ...
    it('#07 => should fail validation with unknown intermediate path in deeply nested structure', () => {
      const schema = v.object({
        system: v.object({
          monitoring: v.object({
            metrics: v.object({ cpu: v.object({ usage: v.number() }) }),
          }),
        }),
      });
      ...
      const decomposed = decomposeSchema(schema);
      // Valid path
      const validData = { 'system.monitoring.metrics.cpu.usage': 75.5 };
      const validResult = v.safeParse(decomposed, validData);
      expect(validResult.success).toBe(true);
    ...
    it('#09 => should handle validation with partial known paths and unknown final keys', () => {
      const schema = v.object({
        api: v.object({
          v2: v.object({
            endpoints: v.object({
              users: v.object({ create: v.string(), read: v.string() }),
            }),
          }),
        }),
      });
    ...
    it('#11 => should fail validation with unknown branch in existing path structure', () => {
      const schema = v.object({
        config: v.object({
          database: v.object({
            primary: v.object({ host: v.string(), port: v.number() }),
            secondary: v.object({ host: v.string(), port: v.number() }),
          }),
        }),
      });
      ...
      const schema = v.object({
        config: v.object({
          environments: v.object({
            production: v.object({
              database: v.object({ host: v.string(), port: v.number() }),
              cache: v.object({ ttl: v.number(), strategy: v.string() }),
            }),
          }),
        }),
      });
