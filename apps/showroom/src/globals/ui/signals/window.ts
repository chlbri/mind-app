...
/**
 * Creates a deferred accessor providing the browser `window` object once
...
/**
 * Mapped object type wrapping window handlers in client-safe caller
 * functions.
 */
type Out<T extends UseWindowProps> = { [K in keyof T]: () => void };

/**
 * Wraps window operations in client-safe invoker functions that only
 * execute when `window` is available.
 *
 * @template | {@linkcode UseWindowProps} `T` - Map of window handler
 *   functions.
 *
