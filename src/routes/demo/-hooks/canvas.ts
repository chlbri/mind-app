...
export const useCanvas = ({ elements, links }: CanvasProps) => {
  ...
  const [_links, setLinks] = createSignal([...links]);
  ...
  const visibleElements = () => {
    ...
    const addVisible = (elem: v.InferInput<typeof ElementSchema>) => {
      visible.add(elem.id);
  ...
  return {
    canvasRef: (el: HTMLCanvasElement) => {
      canvasRef = el;
    },
    selectedId,
    setSelectedId,
