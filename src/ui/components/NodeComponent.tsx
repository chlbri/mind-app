...
export const NodeComponent: Component<Props> = (props) => {
  ...
  const hasParent = useState(service, {
    selector: (s) => {
      const edges = s.context?.data?.edges;
  ...
  return (
    ...
    >
      ...
      >
        <svg
          class="size-6 fill-[#a11111] rounded-full cursor-pointer opacity-100 transition-all duration-200 ease-in-out"
          onClick={(e) => {
            e.stopPropagation();
      ...
      <Show when={props.label} keyed>
        {(label) => <span class="p-3 border-b border-[#f0f0f0] select-none">{label}</span>}
      </Show>
      ...
      <Show when={props.input || hasParent()}>
        <div
          id="outputs"
          class="pointer-events-none cursor-default z-10 absolute top-0 -left-4.5 flex flex-col"
        >
          <div
            ref={(el) => (inputRef = el)}
            class="cursor-default bg-[#e38b29] size-3 rounded-full mt-3 shadow-md"
            onMouseDown={(event) => {
      ...
      <div id="inputs" class="pointer-events-none z-10 absolute top-0 -right-4.5 flex flex-col">
        <div
          ref={(el) => (outputRef = el)}
          class="cursor-crosshair bg-[#e38b29] size-3 rounded-full mt-3 shadow-md"
          style={{ "pointer-events": "all" }}
          onMouseDown={(event) => {
            event.stopPropagation();
