...
export const NodeComponent: Component<Props> = (props) => {
  ...
  return (
    ...
    >
      ...
      >
        <svg
          class="fill-[#a11111] rounded-full cursor-pointer opacity-100 transition-all duration-200 ease-in-out"
          onClick={(e) => {
            e.stopPropagation();
        ...
        <Show when={hasParent()}>
          <svg
            class="bg-green-500 rounded-full p-0.5 hover:bg-green-600 font-bold text-center cursor-pointer overflow-visible"
            style={{
              "pointer-events": "all",
              "fill-rule": "evenodd",
              "clip-rule": "evenodd",
        ...
        <svg
          class="bg-blue-500 text-white p-0.5 rounded-lg hover:bg-blue-600 font-bold text-center flex items-center justify-center cursor-pointer"
          onClick={() =>
            service.send({
              type: "ADD_CHILD",
              payload: props.id,
      ...
      <Show when={props.label} keyed>
        {(label) => (
          <span class="p-3 border-b border-[#f0f0f0] select-none min-w-max text-red-600 whitespace-nowrap">
            {label}
          </span>
        )}
      </Show>
      ...
      <Show when={props.input || hasParent()}>
        <div
          id="outputs"
          class="pointer-events-none cursor-default z-10 absolute top-0 flex flex-col"
          style={{
        ...
        >
          <div
            ref={(el) => (inputRef = el)}
            class="cursor-default bg-[#e38b29] rounded-full shadow-md"
            style={{
      ...
      <div
        id="inputs"
        class="pointer-events-none z-10 absolute top-0 flex flex-col"
        style={{
      ...
      >
        <div
          ref={(el) => (outputRef = el)}
          class="cursor-crosshair bg-[#e38b29] rounded-full shadow-md"
          style={{
            width: `${HANDLE_SIZE}px`,
            height: `${HANDLE_SIZE}px`,
            "margin-top": `${HANDLE_MARGIN_TOP}px`,
