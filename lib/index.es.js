...
import { createDraggable as De, DragDropProvider as Se, DragDropSensors as Ae, DragOverlay as Ie } from "@thisbeyond/solid-dnd";
function Oe(e, n, t) {
...
const q = (e, n) => `edge = ${e} => ${n}`, G = (e) => `node-${e}`, be = Ce(
  {
  ...
  actions: {
    configure: n(
      e("context", ({ context: { data: t } }) => ({ data: t })),
  ...
  () => {
    ...
    ), t = D(), o = D(), [r, l] = D({}, { equals: !1 }), s = be.provideOptions(
        ...
        actions: {
          placeChild: d("context.data.nodes", {
            ADD_CHILD: ({
              ...
              const g = { ...u?.nodes }, p = g[f], w = `node-${h}`, b = e()[f].width, x = p.position.x + b + le;
              return g[w] = {
          ...
          placeSibling: d("context.data.nodes", {
            ADD_SIBLING: ({
              ...
              const p = { ...u?.nodes }, w = h?.find(
                (y) => y.to === f
              )?.from;
              if (!w) return p;
              const b = p[w], x = `node-${g}`, m = e()[w].width, $ = b.position.x + m + le;
              return p[x] = {
                data: { content: "<Nouveau nœud>" },
                input: !0,
                position: { x: $, y: b.position.y + 100 }
              }, p;
            }
          }),
          buildUI: a(
            ...
            c({
                ...
                l(
                  I((u) => {
                    f?.forEach(({ from: h, id: g, to: p }) => {
                      const w = e()[h].output, b = e()[p].input, x = o[0]();
                      b && x && (u[g] = {
                        x0: w.x - x.x + 6,
                        y0: w.y - x.y + 6,
                        x1: b.x - x.x + 6,
              ...
              MOVE: ({ pContext: { edges: f }, payload: u }) => {
                l(
                  I((h) => {
                    f?.forEach(({ from: g, to: p, id: w }) => {
                      if (g === u.id) {
                        const b = e()[u.id].width, x = u.x + b + 9, m = u.y + 19.5;
                        h[w] = {
                          ...h[w],
                          x0: x,
                          y0: m
            ...
            MOVE_IMMEDIATE: ({ pContext: { edges: f }, payload: u }) => {
              l(
                I((h) => {
                  f?.forEach(({ from: g, to: p, id: w }) => {
                    if (g === u.id) {
                      const b = e()[u.id].width, x = u.x + b + 4, m = u.y + 16.5;
                      h[w] = {
                        ...h[w],
                        x0: x,
                        y0: m
...
F(["click"]);
var ze = /* @__PURE__ */ S('<svg class="pointer-events-none absolute top-0 w-full h-full">');
const Ve = () => {
  const [e, n] = D(), {
    newEdge: [t],
    edgesPositions: [o]
...
const We = (e) => {
  ...
  he(() => {
    const g = n, p = t, w = o();
    if (!p || !w) return;
    const b = g?.getBoundingClientRect(), x = p.getBoundingClientRect(), m = w.getBoundingClientRect(), $ = b ? {
      x: b.x,
      y: b.y
    } : void 0, y = {
      x: x.x,
  ...
  const u = d.context((g) => {
    ...
    return p ? Object.values(p).some((w) => w.to === e.id) : !1;
  }), h = De(e.id);
  return (() => {
    var g = Ye(), p = g.firstChild, w = p.firstChild, b = w.nextSibling, x = p.nextSibling, m = x.firstChild;
    j(h, g, () => !0), g.$$mousedown = (y) => {
      ...
      get children() {
        ...
        var A = n;
        return typeof A == "function" ? j(A, v) : n = v, y;
      }
    }), x), m.$$mousedown = (y) => {
      y.stopPropagation(), d.send("DESELECT");
      const v = a(), A = l()[e.id].output;
      v && c({
        x0: A.x - v.x + 6,
        y0: A.y - v.y + 6,
        x1: y.x - v.x,
    ...
    return typeof $ == "function" ? j($, m) : t = m, N((y) => {
      ...
      return y.e = X(g, v, y.e), A !== y.t && W(g, "top", y.t = A), M !== y.a && W(g, "left", y.a = M), y.o = X(p, U, y.o), y;
    }, {
      e: void 0,
      t: void 0,
      a: void 0,
...
var Ke = /* @__PURE__ */ S('<div class="w-full h-full relative">');
const Je = () => {
  const [e, n] = D(), {
    board: [, t],
    service: o
...
F(["mousedown"]);
var Ze = /* @__PURE__ */ S('<div class="relative w-full h-full overflow-hidden"><div class="w-full h-full overflow-scroll"><div class="relative h-[150vh] w-[2160px] bg-white bg-size-[30px_30px]"style="background-image:radial-gradient(circle, #b8b8b8bf 1px, rgba(0, 0, 0, 0) 1px)">');
const it = (e) => {
  const n = {
    "node-0": {
      data: {
